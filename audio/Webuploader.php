...
class Webuploader extends InputWidget{
    ...
    public function init()
    {
        ...
        $this->options['innerHTML'] = isset($this->options['innerHTML']) ? $this->options['innerHTML'] : '<audio controls="controls"> <source src="" type="audio/mpeg"><embed height="50" width="100" src=""></audio><div id="picker_'.$this->options['id'].'" /><button class="btn btn-default" type="button">选择音频</button></div>';
    ...
    public function run()
    {
        ...
        $content = $value ? '<audio controls="controls"> <source src="'.(strpos($value, 'http:') === false ? (\Yii::getAlias('@static') . '/' . $value) : $value).'" type="audio/mpeg"><embed height="50" width="100" src="'.(strpos($value, 'http:') === false ? (\Yii::getAlias('@static') . '/' . $value) : $value).'"></audio><div id="picker_'.$this->options['id'].'"><button class="btn btn-default" type="button">替换音频</button></div>'
	        :
    ...
    private function registerLocalClientJs()
    {
        ...
        $server = $this->server ?: Url::to(['/site/audio']);
...
JS
    ...
    {
        WebuploaderAsset::register($this->view);
        $tokenUrl = $this->server ?: Url::to(['/site/audio']);
        $swfPath = \Yii::getAlias('@webuploader/assets');
        $this->view->registerJs(<<<JS
...
uploader.on( 'uploadProgress', function( file, percentage ) {
    ...
    uploader.on( 'uploadSuccess', function( file, data ) {
        var url = data.key;
        $( '#picker_{$this->options['id']}' ).next().fadeOut();
        $( '#picker_{$this->options['id']}' ).prev().find("source").attr('src','{$this->domain}/'+url);
        $( '#picker_{$this->options['id']}' ).prev().find("embed").attr('src','{$this->domain}/'+url);
