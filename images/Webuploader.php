...
class Webuploader extends InputWidget{
    ...
    public function init()
    {
        ...
        $this->options['innerHTML'] = isset($this->options['innerHTML']) ? $this->options['innerHTML'] : '<img src="/admin/img/nopic.jpg" height="150"/><div id="picker_'.$this->options['id'].'"><button class="btn btn-default" type="button">选择图片</button></div>';
    ...
    public function run()
    {
        ...
        $content = $value ? '<img src="'.(strpos($value, 'http:') === false ? (\Yii::getAlias('@static') . '/' . $value) : $value).'" height="150"/><div id="picker_'.$this->options['id'].'"><button class="btn btn-default" type="button">替换图片</button></div>'
            :
    ...
    private function registerLocalClientJs()
    {
        ...
        $server = $this->server ?: Url::to(['/site/image']);
