...
class Webuploader extends InputWidget{
    ...
    public function init()
    {
        ...
        $this->options['innerHTML'] = isset($this->options['innerHTML']) ? $this->options['innerHTML'] : '<a href="javascript:;"><img src="/admin/img/nopic.jpg" width="140" height="140"/></a><div id="picker_'.$this->options['id'].'" /><button class="btn btn-default" type="button">选择文件</button></div>';
    ...
    public function run()
    {
        ...
        $content = $value ? '<a href="'.(strpos($value, 'http:') === false ? (\Yii::getAlias('@static') . '/' . $value) : $value).'"><img src="/admin/img/file.jpg" width="140" height="140"/></a><div id="picker_'.$this->options['id'].'"><button class="btn btn-default" type="button">替换文件</button></div>'
	        :
    ...
    private function registerLocalClientJs()
    {
        ...
        $server = $this->server ?: Url::to(['/site/file']);
...
JS
    ...
    {
        WebuploaderAsset::register($this->view);
        $tokenUrl = $this->server ?: Url::to(['/site/file']);
        $swfPath = \Yii::getAlias('@webuploader/assets');
        $this->view->registerJs(<<<JS
